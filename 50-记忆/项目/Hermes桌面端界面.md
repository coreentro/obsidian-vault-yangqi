...
- `2026-08-17` **改用左右分栏（你右 / AI 左）** — 用户否决了「加标签+色条」方案，要微信式左右分开。位置差异比颜色差异更直观。
- `2026-08-17` **不动用户 root 的宽度，只收窄内部气泡** — root 是 `sticky` + 满宽 + `-mx-4`（粘顶条机制），改它的 width 会让 sticky 背景无法横向铺满。
- `2026-08-17` ~~只强化用户消息，助手消息仅加淡左边界~~（已废弃） — 助手消息含大量 Markdown（表格/代码块），重度改样式易破版。
...
- `2026-08-17` **按用户反馈删除 `chat-role-contrast`**，改建 `chat-sides`（你右 / AI 左）：
  ...
  - 不再使用 `::before` 文字标签（用户不喜欢）
  - 验证：7 个选择器/类名全部命中（含 `composer-human-message` 6 处）、3 个变量有定义、ESM 可加载
...
- **`chat-role-contrast`（左色条 + 「你」/「HERMES」文字标签）→ 用户明确否决**（"不好，删了吧"）。已删除。教训：用户要的是**空间位置区分**，不是加装饰元素。
