---
title: "非程序员的外挂：API"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/HOhCwzEYtiVSdSkFbfOcghxHnIf
node_token: HOhCwzEYtiVSdSkFbfOcghxHnIf
obj_token: WuuGdAueeo9iihxE4JkcXLnon4g
obj_type: docx
...
**PS：这一节中关于Coze的API接入可以不用看，主要看API的定义，看完就可以看下一节了**
...
<whiteboard token="GAhjwzPaah3n23bgQ4fcqKLfnud"></whiteboard>
...
> [!abstract]- 🖼 图片展示的是一个应用程序界面中的模型设置部分。在“模型”下拉框中，选中了
> 图片展示的是一个应用程序界面中的模型设置部分。在“模型”下拉框中，选中了“豆包·Function call模型 32K”。下方有多个模型选项，其中“Moonshot（8K）”“Moonshot（32K）”“Moonshot（128K）”被红色框突出显示。该图片与上下文关系紧密，上下文在介绍API组成三要素时，用生活中容易理解的场景类比，此处可能是在说明API中模型这一要素的具体呈现形式。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/RZ2abAJefoLAPjxruaVc3cqsnXe) · `RZ2abAJefoLAPjxruaVc3cqsnXe`
...
> [!abstract]- 🖼 图片展示了非程序员使用API发起对话的基础信息。其中，请求方式为POST
> 图片展示了非程序员使用API发起对话的基础信息。其中，请求方式为POST；请求地址为https://api.coze.cn/v3/chat；权限为chat，需确保调用该接口使用的个人令牌开通此权限；接口说明为调用此接口发起一次对话，支持添加上下文和流式响应。该图片与上下文紧密相关，是对文档中API请求路径定义和请求方式相关内容的直观呈现，帮助非程序员快速了解API调用的基础配置。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/X9PTbXZTqorSB5xVlk8cioLZnbd) · `X9PTbXZTqorSB5xVlk8cioLZnbd`

## 请求方式

> [!abstract]- 🖼 图片展示了API请求方式为POST的内容。在“请求方式”处以红色框突出显
> 图片展示了API请求方式为POST的内容。在“请求方式”处以红色框突出显示“POST”，下方对应请求地址为“https://api.coze.cn/v3/chat”，权限为“chat”，接口说明为“调用此接口发起一次对话，支持添加上下文和流式响应”。该图片与文档中介绍API请求方式的内容相关，直观呈现了POST请求方式在API接口中的具体表现形式。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/XThbb0MmxozJZUxz52UcAcLEnkf) · `XThbb0MmxozJZUxz52UcAcLEnkf`
...
> 图片展示了API请求地址的相关信息。其中“请求地址”栏以红色框突出显示，内容为“https://api.coze.cn/v3/chat”，表明这是API通信时确定应用程序位置的关键部分。该图片与上下文紧密相关，上下文在介绍API请求路径定义时，将请求地址拆解为通信协议、域名和接口路径三部分，此图直观呈现了请求地址的具体内容，帮助理解其在API通信中的作用。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/EUHhb1aF0oru6bxhGgPcRtO3nVh) · `EUHhb1aF0oru6bxhGgPcRtO3nVh`
...
> 图片展示的是一个查询IP地址的网页界面。上方显示“我的IP地址是：124.160.201.181”，并有“其他IP查询”和“此处广告位招租”两个蓝色文字选项。下方表格中，左侧“IP”栏显示124.160.201.181，“IPv6”栏显示::7ca0:c9b5。右上角有“查看自己的IP地址”蓝色文字。该图片与文档中介绍IP地址相关，用于直观呈现查询IP地址的结果，帮助理解互联网中服务器通过公网IP地址定位的原理。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JubsbCUMaowajFxBhAvcZw62nch) · `JubsbCUMaowajFxBhAvcZw62nch`
...
> 图片展示了Coze API接口的相关信息。其中，“权限”部分以红色框突出显示，明确指出调用该接口使用的个人令牌需开通“chat”权限，详情可参考“鉴权”。该图片与文档中“权限”部分内容对应，直观呈现了使用Coze API接口时对权限的具体要求，帮助开发者了解在调用相关接口前需确保个人令牌已开通相应权限。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/FSTjbFsYqosaH0xITGucc8ZHnYd) · `FSTjbFsYqosaH0xITGucc8ZHnYd`
...
> [!abstract]- 🖼 图片展示了Coze API的基础信息。请求方式为POST，请求地址是ht
> 图片展示了Coze API的基础信息。请求方式为POST，请求地址是https://api.coze.cn/v3/chat，权限为chat。接口说明部分用红色框突出显示，明确指出调用此接口可发起一次对话，支持添加上下文和流式响应。该图片与文档中接口说明部分对应，直观呈现了接口的请求方式、地址、权限及主要功能，帮助使用者快速了解接口的基本信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/P7KJbncqKod3ocxtPH3c2VVBn7g) · `P7KJbncqKod3ocxtPH3c2VVBn7g`
...
> [!abstract]- 🖼 图片展示了API调用中Header部分的输入参数说明。表格包含参数、取值
> 图片展示了API调用中Header部分的输入参数说明。表格包含参数、取值、说明三列。Authorization参数取值为Bearer {Personal_Access_Token}，用于验证客户端身份的访问令牌，可在扣子平台生成；Content-Type参数取值为application/json，解释请求正文的方式。该图与上下文紧密相关，上下文介绍了Header的主要用途及输入方式，此图是对Header输入参数的具体说明。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JOCHbMzGhoC0wcxrI37cIfvanPe) · `JOCHbMzGhoC0wcxrI37cIfvanPe`
...
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/S9ANboVPWoZM2ex6CQIcuKSBnRf) · `S9ANboVPWoZM2ex6CQIcuKSBnRf`
...
> [!abstract]- 🖼 图片展示了API接口调用示例，请求方式为GET，请求地址为https:/
> 图片展示了API接口调用示例，请求方式为GET，请求地址为https://api.coze.cn/v1/space/published_bots_list。权限为getPublishedBot，需确保调用该接口使用的个人令牌开通此权限。接口说明为调用接口查看指定空间发布到Bot as API渠道的Bot列表。该图片与上下文紧密相关，是对上文“GET请求&Query形式的参数”示例的具体呈现，直观展示了GET请求的请求方式及请求地址等关键信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/MZptbyLlKofG9mxCQ8scS1zSn8g) · `MZptbyLlKofG9mxCQ8scS1zSn8g`

> [!abstract]- 🖼 图片展示了API GET请求中Query形式的参数示例。表格中包含参数、
> 图片展示了API GET请求中Query形式的参数示例。表格中包含参数、类型、是否必选及说明四列。关键参数有space_id，类型为String，必选，是Bot所在空间的唯一标识；page_size，类型为Integer，可选，分页大小，默认为20；page_index，类型为Integer，可选，分页查询时的页码，默认为1。该图片与上下文紧密相关，直观呈现了GET请求中Query参数的结构及具体参数信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/S8NcbDgogoDiGbxuzwXcSnDxnkd) · `S8NcbDgogoDiGbxuzwXcSnDxnkd`
...
> 图片展示了API的请求方式、请求地址、权限及接口说明等信息。其中，请求方式为POST；请求地址为https://api.coze.cn/v3/chat；权限为chat，需确保调用该接口使用的个人令牌开通了chat权限；接口说明是调用此接口发起一次对话，支持添加上下文和流式响应。该图片与文档中介绍API入参的内容相关，直观呈现了API调用的基本配置信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/GKu0bkJ9mowJVgx9EFXcoy82n3b) · `GKu0bkJ9mowJVgx9EFXcoy82n3b`

> [!abstract]- 🖼 图片展示了API中Query部分的参数说明。参数包括conversati
> 图片展示了API中Query部分的参数说明。参数包括conversation_id，类型为String，可选，说明标识对话发生在哪一次会话中，一个会话包含一条或多条消息，对话是会话中对Bot的一次调用，Bot会将对话中产生的消息添加到会话中，且一个会话中只能有一个进行中的对话，否则调用此接口时会报错4016。该图片与上下文紧密相关，是对API输入参数中Query部分的详细解释，帮助开发者理解相关参数的含义及使用场景。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/E9f9bAROXoIOWBxtYKLc1UEXnSe) · `E9f9bAROXoIOWBxtYKLc1UEXnSe`

> [!abstract]- 🖼 图片展示了API中Query部分的参数说明。参数包括conversati
> 图片展示了API中Query部分的参数说明。参数包括conversation_id，类型为String，是否必选为可选，说明标识对话发生在哪一次会话中，一个会话包含一条或多条消息，对话是会话中对Bot的一次调用，Bot会将对话中产生的消息添加到会话中，且一个会话中只能有一个进行中的对话，否则调用此接口时会报错4016。该图片与文档中介绍API入参的内容相关，是对Query部分参数的详细说明。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/QmBkbGgv3oPNHgxiWmfczKMqnsg) · `QmBkbGgv3oPNHgxiWmfczKMqnsg`

PS：**如果文档中标注的是Body参数，代表这些参数是通过JSON的方式输入的**

这时候大家一定要先去了解下JSON的格式哦：[[Coze中的复杂数据类型（Object与Array）]]
...
非流式输出的内容就是JSON：：[[30-资源/课程/Coze实战课程/10-课本章/成为Agent工程师/Coze实战课项目/加餐文档/成为Agent工程师之AI编程/09｜程序中的变量|09｜程序中的变量]]
...
> 图片展示的是Coze平台发布Bot的界面。在“选择发布平台”部分，有多个平台选项，如抖音、飞书等，其中“Bot as API”选项被红色框线突出显示，其状态为“已提交”。右上角有“发布”按钮。该图片与文档中“在个人空间创建一个Bot，发布的时候勾选Bot API”的内容相关，直观呈现了发布Bot时选择“Bot as API”这一操作步骤。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JdEmbQ2Opot02fxXDQScteqQnNh) · `JdEmbQ2Opot02fxXDQScteqQnNh`

1. Coze发布成功之后，**在个人空间，点击刚刚发布的Bot，一定要在个人空间哦**

> [!abstract]- 🖼 图片展示了Coze平台个人空间中Bot管理界面。左侧导航栏选中“个人空间
> 图片展示了Coze平台个人空间中Bot管理界面。左侧导航栏选中“个人空间”，右侧显示多个Bot，其中“小红书API测试”Bot被红框突出显示，其下方有“豆包 - Function call模型”“最近编辑 00:13”“大圣 @lmh_2024”等信息。该图片与上文“在个人空间，点击刚刚发布的Bot，一定要在个人空间哦”的内容对应，用于指导用户找到并点击发布成功的Bot。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/HMJNbeK5iows7bxMyytcDy5Unzc) · `HMJNbeK5iows7bxMyytcDy5Unzc`

> [!abstract]- 🖼 图片展示了在浏览器中打开Coze Bot的页面，地址栏内容为“https
> 图片展示了在浏览器中打开Coze Bot的页面，地址栏内容为“https://www.coze.cn/space/7331741379686826035/bot/7393008672953679913”。画面中有一个红色框突出显示地址栏内容，并有红色箭头指向，旁边有文字提示“复制地址栏内容”。这与文档中“在个人空间，点击刚刚发布的Bot，一定要在个人空间哦”及“提取最后面那串数字：7393008672953679913”的内容相关，用于指导用户复制Bot的地址栏内容。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/IrmNbT7P2onFTyxmWMOcR9UpnIb) · `IrmNbT7P2onFTyxmWMOcR9UpnIb`
...
> 图片展示的是Coze平台中个人访问令牌管理界面。界面上方有“个人访问令牌”“OAuth应用”“已授权应用”三个选项卡，当前选中“个人访问令牌”。下方列表显示了两个令牌信息，包括名称、创建时间、最近使用时间、过期时间、状态及操作栏。右上角有“添加新令牌”蓝色按钮，下方有红色箭头指向该按钮，提示点击添加新令牌。此图与文档中申请Coze Token部分内容相关，用于说明申请Token的操作位置。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JGuBbTgO9ovBDQxH3RjcWUbbnBf) · `JGuBbTgO9ovBDQxH3RjcWUbbnBf`

> [!abstract]- 🖼 图片展示了在Coze中申请个人访问令牌的界面。名称处填写“Secret 
> 图片展示了在Coze中申请个人访问令牌的界面。名称处填写“Secret token”，过期时间选择“永久有效”，访问团队空间选“所有团队空间”，权限部分勾选“Bot”“chat”“getMetadata”。下方有“取消”和“确定”按钮，其中“确定”按钮被红色框突出显示。该图片与文档中“申请自己的Coze Token”部分内容相关，是申请Token时的操作界面展示，需将Token用于后续API调用。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/SE2IboS16oV2lxxZIXzcCOi1njb) · `SE2IboS16oV2lxxZIXzcCOi1njb`

> [!abstract]- 🖼 图片展示了Coze平台申请个人访问令牌的界面。界面中强调令牌仅显示一次，
> 图片展示了Coze平台申请个人访问令牌的界面。界面中强调令牌仅显示一次，需保存在安全且可获取的地方，不要与他人共享或在代码中暴露。下方有“名称”“过期时间”“令牌”等信息栏，其中“令牌”处显示了长字符串，右侧有一个文件夹图标。图片与文档中“申请自己的Coze Token”部分内容相关，是申请Token时的展示界面，用于说明获取Token的过程。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/YoBGbBVI4oXIDfxMUBCc9vuynDd) · `YoBGbBVI4oXIDfxMUBCc9vuynDd`
...
> [!abstract]- 🖼 图片展示了API接口测试工具中调用Coze API的设置界面。关键信息有
> 图片展示了API接口测试工具中调用Coze API的设置界面。关键信息有：请求地址为https://api.coze.cn/v3/chat，需注意HTTPS；请求方式为POST；Header设置有Content-Type和Authorization；入参为JSON串，需设置；返回值在下方显示。该图与上文介绍的调用Coze API查看消息的步骤相关，直观呈现了操作设置及返回结果，帮助用户了解如何在该工具中进行API调用。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/WcGqb2B8zomib5xhunAcSQiCnLd) · `WcGqb2B8zomib5xhunAcSQiCnLd`
...
> [!abstract]- 🖼 图片展示了API测试工具界面中“HTTP接口测试”页面，用于测试http
> 图片展示了API测试工具界面中“HTTP接口测试”页面，用于测试https://api.coze.cn/v3/chat/message/list接口。关键信息有：请求地址为HTTPS链接；请求方式为GET；Header设置有Content-Type和Authorization；参数设置为Query形式，包含conversation_id和chat_id；返回值为JSON格式，显示了对话内容等信息。该图与上文介绍的API测试内容紧密相关，直观呈现了测试时的具体操作和返回结果。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/F2k6bYO9OoZywVxl0YXc9e6jnHe) · `F2k6bYO9OoZywVxl0YXc9e6jnHe`
...
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/W75ibSCKFoNVTpxVfcucvncEnW7) · `W75ibSCKFoNVTpxVfcucvncEnW7`
