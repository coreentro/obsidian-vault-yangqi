---
title: "【选修】理论篇：程序中的API是什么"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/FfmLwzjCpigWcZkLgu1c4pJvnDY
node_token: FfmLwzjCpigWcZkLgu1c4pJvnDY
obj_token: NxbedcOBponUp8xUSI3cZ7nLnLf
obj_type: docx
...
<whiteboard token="OSkOwcJXPhVKzIbjrapcRBT0nI2"></whiteboard>
...
> [!abstract]- 🖼 图片展示了程序间通过API通信的示意图。左侧是人形图标“你”，向中间的A
> 图片展示了程序间通过API通信的示意图。左侧是人形图标“你”，向中间的AI Agent（某个AI工具）下指令。中间的AI Agent通过API与右侧的DeepSeek服务器通信，服务器内有大模型运行。API作为程序间的“接口人”，负责传递消息，箭头表示信息传递方向。底部文字说明AI Agent工具通过API把问题传给DeepSeek，再把回答拿回来，API是程序间的“接口人”，负责传递消息。此图与上下文介绍的Agent工具通过API与DeepSeek服务器通信，将问题传给大模型并拿回回答的内容相契合。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/N66jbGMkSoXF67xSYW3c0giJnNf) · `N66jbGMkSoXF67xSYW3c0giJnNf`

当你在网页上跟 DeepSeek 聊天的时候，其实浏览器的背后也是在通过 API 跟 DeepSeek 的服务器通信。

只不过浏览器帮你把这个过程藏起来了，你感知不到而已

> [!abstract]- 🖼 这张图片是关于人、浏览器与DeepSeek服务器间通信的示意图，展示了A
> 这张图片是关于人、浏览器与DeepSeek服务器间通信的示意图，展示了API的实际应用过程。图中左侧的小人代表使用者，箭头指向中间的浏览器（网页）。使用者可在浏览器中操作，在发送问题后，浏览器会按照约定的格式，通过API向右侧的DeepSeek服务器发送问题，服务器处理后再通过API将回答返回给浏览器，呈现给使用者。图片对应文档中关于API的内容，直观呈现了网页与DeepSeek服务器借助API通信的流程，也呼应了文档中提到的浏览器隐藏了这一通信过程、使用者感知不到的说明。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/E14ybcRSCo3k7ixj0Z2cVToMnjc) · `E14ybcRSCo3k7ixj0Z2cVToMnjc`
...
> [!abstract]- 🖼 图片展示了API通信的三要素，分别为请求地址、输入参数、返回结果。请求地
> 图片展示了API通信的三要素，分别为请求地址、输入参数、返回结果。请求地址是去哪里找这个程序，输入参数是你想跟这个程序说什么，返回结果是这个程序给你的回复。图中用“你（调用方）”与“DeepSeek服务器”进行类比，类比为跟朋友打电话，先拨号（请求地址）→说你的事（输入参数）→听对方回复（返回结果）。该图与上下文紧密相关，通过生活中的打电话类比，直观呈现了API通信的三要素。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/BZxtbbiAZoWoTqxK7Spcn6NQnXb) · `BZxtbbiAZoWoTqxK7Spcn6NQnXb`
...
> [!abstract]- 🖼 图片展示了请求地址的拆解，以“https://api.deepseek.
> 图片展示了请求地址的拆解，以“https://api.deepseek.com/chat/completions”为例。其中“https://”是通信协议，即程序之间说话的“语言”；“api.deepseek.com”是域名，找到DeepSeek的服务器；“/chat/completions”是接口路径，定位到“对话”功能。底部总结为：定位到某台服务器的某个功能，就像找到某栋楼（域名）里的某个房间（接口路径）。该图与上下文紧密相关，直观说明了请求地址中各部分的作用及含义。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/OOq7bavEOobjcTxZcWFcRyf4ndh) · `OOq7bavEOobjcTxZcWFcRyf4ndh`
...
> [!abstract]- 🖼 该图片展示了DeepSeek开放平台的页面，对应实操步骤中获取API K
> 该图片展示了DeepSeek开放平台的页面，对应实操步骤中获取API Key的环节。页面左侧导航栏的高亮标识了当前处于“API keys”板块，页面主区域的“用量信息”部分，清晰标注了充值余额为99.84元人民币，本月消费为0.00元人民币，页面顶部还显示了DeepSeek的品牌标识及平台相关的功能选项，该页面是创建与保存DeepSeek API Key的关键操作页面。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/H1MUbVN4ooGo4mxWVQfcWW0en0e) · `H1MUbVN4ooGo4mxWVQfcWW0en0e`

1. **在左侧菜单找到 API Keys**
2. 点击**创建 API Key**，系统会生成一串类似 `sk-xxxxxxxxxxxxxxxx` 的字符串

> [!abstract]- 🖼 图片展示的是DeepSeek开放平台的API keys页面。页面左侧有“
> 图片展示的是DeepSeek开放平台的API keys页面。页面左侧有“用量信息”“充值”“账单”等选项，当前选中“API keys”。右侧显示API keys列表，包含名称、Key、创建日期和最新使用日期等信息，其中“日常使用”Key为“sk - 3738f************56ce”，创建日期为2025 - 03 - 08，最新使用日期也为2025 - 03 - 08。页面底部有“创建API key”按钮。该图片与上下文紧密相关，直观呈现了获取DeepSeek API Key的操作界面及关键信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JPbmbx5X7oq1atxnRpHcKoKVnxd) · `JPbmbx5X7oq1atxnRpHcKoKVnxd`
...
> [!abstract]- 🖼 图片展示了DeepSeek API的相关信息。其中，base_url为h
> 图片展示了DeepSeek API的相关信息。其中，base_url为https://api.deepseek.com，api_key需申请获取。特别指出，deepseek-chat对应DeepSeek-V3.2的非思考模式，deepseek-reasoner对应DeepSeek-V3.2的思考模式。此外，还提到出于与OpenAI兼容考虑，base_url可设置为https://api.deepseek.com/v1，但v1与模型版本无关。该图片与上下文紧密相关，是对上文提到的在文档里找到请求地址、参数格式等内容的补充说明。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/TXacbqQ6wovVTnx2Ovfc0NOjnPD) · `TXacbqQ6wovVTnx2Ovfc0NOjnPD`
...
> [!abstract]- 🖼 图片展示了调用对话API的curl命令示例。命令用于访问DeepSeek
> 图片展示了调用对话API的curl命令示例。命令用于访问DeepSeek API，请求地址为https://api.deepseek.com/chat/completions，需添加Content-Type和Authorization头信息，其中Authorization值为Bearer后的API Key。请求体为JSON格式，包含model、messages（系统和用户信息）和stream（false）参数。该图片与上文介绍调通API时在文档里找到请求地址、参数格式、调用示例等内容相呼应，直观呈现了实际操作示例。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DZKNbTwaWoiFcZxw6gUcS1DQnAb) · `DZKNbTwaWoiFcZxw6gUcS1DQnAb`
...
> [!abstract]- 🖼 图片展示的是用curl命令调用DeepSeek API的示例代码。代码中
> 图片展示的是用curl命令调用DeepSeek API的示例代码。代码中关键部分是将`${DEEPSEEK_API_KEY}`替换为API Key，该部分用红色框突出显示。上下文提到curl是一个命令行工具，用于发送网络请求，此图即为用curl命令调用API的命令示例，直观呈现了命令格式，帮助读者理解如何在终端输入命令进行API调用。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DLURbkPMnoylKWx9KVpc6aUzn9f) · `DLURbkPMnoylKWx9KVpc6aUzn9f`
...
> [!abstract]- 🖼 图片展示了使用curl命令调用DeepSeek API后的输出结果。命令
> 图片展示了使用curl命令调用DeepSeek API后的输出结果。命令中包含请求地址、权限验证、输入参数等信息。输出结果中，红色箭头指向部分显示了API的响应内容，其中“content”字段显示“你好，我是DeepSeek，一个由深度求索公司创造的AI助手，致力于用热情和细心为你提供帮助！😊” ，即为对“你好，请用一句话介绍你自己”这一问题的回答。该图片与上文介绍curl命令调用API及拆解命令内容相呼应，直观呈现了调通API后的结果。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DaYuboZVbo22EXxnMRkcjT2wn0e) · `DaYuboZVbo22EXxnMRkcjT2wn0e`
