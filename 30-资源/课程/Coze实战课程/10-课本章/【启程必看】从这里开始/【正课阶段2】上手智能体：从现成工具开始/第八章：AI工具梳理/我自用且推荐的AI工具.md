---
title: "我自用且推荐的AI工具"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/RTfWwTFyliqK5wkNJqtcXYESnob
node_token: RTfWwTFyliqK5wkNJqtcXYESnob
obj_token: A2Wqd9RTbowLfNxZF0Pc6kR7nHf
obj_type: docx
space_id: 7491877341887725572
space_name: "Agent工程师之Coze实战课程"
depth: 3
breadcrumb:
  - "【启程必看】从这里开始"
  - "【正课阶段2】上手智能体：从现成工具开始"
  - "第八章：AI工具梳理"
  - "我自用且推荐的AI工具"
obj_create_time: 
obj_edit_time: 
creator: 
owner: 
revision_id: 67
from_group_share: false
migrated_from: 飞书云文档
migrated_at: 2026-07-29
tags:
  - 飞书迁移
  - 【启程必看】从这里开始
---

# 我自用且推荐的AI工具

> [!info] 位置
> 【启程必看】从这里开始 › 【正课阶段2】上手智能体：从现成工具开始 › 第八章：AI工具梳理

> [!warning]- 🎬 视频（`video/mp4`）
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/OUDpbZ270oqn6bxGtNrcL66qnAf) · `OUDpbZ270oqn6bxGtNrcL66qnAf`

# 写在前面

你好，我是大圣

这节课给大家分享一下我自用且推荐的一些工具

# 工具1：网络工具

<callout emoji="✨"><p>[第三节：方案一·买一个现成的网络工具](https://axsppz4oyvj.feishu.cn/wiki/NAKhwOoW3imlOHkEP9Wcsi3inJh)</p><p>[第五节：方案三·中转+静态IP一体化方案](https://axsppz4oyvj.feishu.cn/wiki/JX9mwHciWinqbQk3vuScx6LNnPe)</p></callout>

# 工具2：CodeX + Claude Code

我是做的是 AI时代的一人公司，因此强大的模型是我们最重要的伙伴

关于 CodeX 和 Claude Code，后面会有详细的教程

# 工具3：语音转文本工具

这本质不是一个工具，而是一种与 AI交互的理念

请先看下面这篇文章

[[语音转文本工具：AI时代的沟通利器]]

[[如何利用AI，20分钟把碎碎念，变成2000字高质量长文]]

> [!warning]- 🎬 视频（`video/mp4`）
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/YHgTbjtmcops0PxNgS7cpwJDnIe) · `YHgTbjtmcops0PxNgS7cpwJDnIe`

# 工具4：闲鱼

<callout emoji="✨">
你没有看错，我把闲鱼APP加在了我的工具里面，我必须让大家在AI时代养成一个意识
当你遇到像网络、账号、APIKey等一系列卡点的时候，**都可以去闲鱼上搜一搜有没有答案**
正确使用闲鱼的逻辑是：当你想要买账号、买代充的时候，**你先把这个事了解清楚，不管是通过互联网的教程，还是通过AI**
你把这个事了解清楚之后，就可以在闲鱼上去买资源
因为你是真的了解这个事情的，所以你可以去向商家问出问题，了解他们的售后情况，确认资源是否符合你的需求
**PS：各位买东西的时候，不要找我推荐靠谱渠道哈**
</callout>

> [!warning]- 🎬 视频（`video/mp4`）
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/LAKkbr7MToFzu5xPtvicSsQFnmg) · `LAKkbr7MToFzu5xPtvicSsQFnmg`

# 工具5：Arc浏览器（可选）

我用的是Arc浏览器

它的好处是：

- 标签页管理更清爽，不会开几十个标签页找不到
- 侧边栏可以固定常用工具，效率高
- 界面干净，适合长时间工作

**但这个不强制**

如果你没有折腾精神，或者你对现在的浏览器很习惯，就不用换

> [!warning]- 🎬 视频（`video/mp4`）
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/EVxRbWNQho20K7xgeAAcyMi6nUb) · `EVxRbWNQho20K7xgeAAcyMi6nUb`

[[我的浏览器【Arc浏览器】]]

# 工具6：我的写作系统

我正在系统地写一系列关于我的写作方法论的文章。

目前已经写了三篇，已经是一个非常完善体系的写作方法论，感兴趣的可以看一下。

[[使用飞书搭建自己AI时代的知识管理体系]]
