---
title: "11｜用LLM编写Coze代码节点"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/CYF1wtmoKiGf6ckxkppcP9DUnvh
node_token: CYF1wtmoKiGf6ckxkppcP9DUnvh
obj_token: SJXPdlxROo1mQsxoR8ecYa7ynrO
obj_type: docx
...
前面我们讲了很多的编程理论和基础知识，从今天这一章开始，我们就正式进入到AI编程的实战阶段。
...
这里我要跟大家泼一盆冷水，在未来很长的一段时间内，**普通人**利用AI编程做出**生产级别可用**的复杂系统都是非常困难的。**除非你自学编程知识，达到了程序员的级别**
...
Coze平台的出现给了普通人打造一款产品的可能性，Coze中有丰富的插件以及功能节点，让非编程人士可以像搭积木一样实现某个具体的功能。同样的，Coze中也提供了代码节点，我们可以通过代码节点编写代码，实现特定的功能，从而让我们的工作流更加稳定和强大
...
<callout emoji="🥛">
Coze中目前支持两种编程语言Python和Javascript，教程全部以Python为语言进行讲解
</callout>
...
分析完Coze的代码节点，你会发现它非常契合我们前面讲到的输入、处理和输出的框架，我把上述的代码模版再抽象一层如下，想必你对这个模版的理解会更加清晰
...
既然要利用AI编程，并且还对输出有一定的要求，那我们非常有必要编写一个提示词来辅助我们
...
PS：由于Claude是目前公认编程能力最强的AI，所以AI编程建议使用Claude。但是由于费用限制，国内的智谱和豆包也还行，具体我没有做大量的测试，请在使用过程中注意！
...
注意：在代码编写完成后，不要忘记将输出的格式调整为和JSON结构一致

> [!abstract]- 🖼 图片展示了Coze代码节点中“两个数组合并”案例的代码及输出结果。代码部
> 图片展示了Coze代码节点中“两个数组合并”案例的代码及输出结果。代码部分，使用Claude大模型生成，将imageInputList和storyList合并成一个数组。输出结果部分，resultList为数组，包含desc、prompt、title、imageUrl四个字符串类型的变量，与代码中变量类型一致，且有“新增”按钮可添加更多变量。该图片与上下文紧密相关，直观呈现了代码节点处理逻辑的输出情况。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DgBpbE1seoElGNxlYO4cNroJn1b) · `DgBpbE1seoElGNxlYO4cNroJn1b`
...
> [!abstract]- 🖼 图片展示了用LLM编写Coze代码节点的编程案例中，字符串转数组的代码编
> 图片展示了用LLM编写Coze代码节点的编程案例中，字符串转数组的代码编写过程。先是提示AI生成代码，AI给出代码后，作者指出代码存在错误，需将输出格式调整为JSON结构。接着作者给出正确代码，包括导入json模块、定义函数、使用json.loads()函数将字符串转换为列表等。最后再次提示AI生成代码，AI给出正确代码，作者指出代码已符合要求。此图与上下文紧密相关，直观呈现了代码编写及调试过程。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/UYZYbmwSMokouYxKLGSczxn3nmg) · `UYZYbmwSMokouYxKLGSczxn3nmg`
