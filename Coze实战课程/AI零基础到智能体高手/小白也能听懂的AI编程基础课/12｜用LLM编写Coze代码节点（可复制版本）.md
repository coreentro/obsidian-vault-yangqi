---
title: "12｜用LLM编写Coze代码节点（可复制版本）"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/K1GEwLBgxiT0pqk5FErcFW9Xnbc
node_token: K1GEwLBgxiT0pqk5FErcFW9Xnbc
obj_token: P29hdB0tDofAMQxs0UFctwCzn5f
obj_type: docx
...
![图片展示了使用Coze代码节点进行编程案例中“两个数组合并”处理逻辑的代码及输出结果。代码部分，输入为两个数组，输出为合并后的数组，代码中使用了`resultList`变量存储合并结果。输出部分，以JSON格式呈现，包含`resultList`数组，其元素为包含`desc`、`prompt`、`title`和`imageUrl`四个字段的对象。该图片与上下文紧密相关，直观呈现了代码节点处理逻辑的实现效果。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NzEzMjA4MmJiN2NkYmQ3MTgxMjkwYmUzMzQ3ZWE0OTFfMmRmNDU5YmQ5ZDFiN2E5MDU2ZmY1MGVkOTk3NjcwNDNfSUQ6NzQ0ODQ3MTk4NDc1NDgwMjY5MV8xNzg1Mjk5NjUxOjE3ODUzMDMyNTFfVjM)
