---
title: "程序中的API是什么"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/FHMYwB7epiTJt1kirNTcLiLMn1f
node_token: FHMYwB7epiTJt1kirNTcLiLMn1f
obj_token: HWaHdDSEboAb9Uxo9Bsc8SOlncf
obj_type: docx
...
> [!abstract]- 🖼 图片展示了程序间通过API通信的流程。左侧是 已读未答
> 图片展示了程序间通过API通信的流程。左侧是 已读未答
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/YzTubN9ovo99msxgzz1cqgbknac) · `YzTubN9ovo99msxgzz1cqgbknac`

当你在网页上跟 DeepSeek 聊天的时候，其实浏览器的背后也是在通过 API 跟 DeepSeek 的服务器通信。

只不过浏览器帮你把这个过程藏起来了，你感知不到而已

> [!abstract]- 🖼 图片展示了人跟应用程序通信的过程。左侧是一个简笔小人代表“你”，中间是浏
> 图片展示了人跟应用程序通信的过程。左侧是一个简笔小人代表“你”，中间是浏览器（网页）界面，右侧是DeepSeek服务器。箭头从“你”指向浏览器，标注“发送”；从浏览器指向服务器，标注“发送问题”；从服务器指向浏览器，标注“返回回答”。下方文字说明在网页上跟DeepSeek聊天时，浏览器背后是通过API跟服务器通信，只是浏览器将该过程隐藏，用户感知不到。此图与上下文结合，直观呈现了API在通信中的作用。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/T0cObrBYLoQFhIx0edRcbh79nlo) · `T0cObrBYLoQFhIx0edRcbh79nlo`
...
> [!abstract]- 🖼 图片展示了API通信的三要素，分别为请求地址、输入参数和返回结果。请求地
> 图片展示了API通信的三要素，分别为请求地址、输入参数和返回结果。请求地址是去哪里找这个程序，输入参数是你想跟这个程序说什么，返回结果是这个程序给你的回复。图中用“你（调用方）”和“DeepSeek服务器”进行类比，类比为跟朋友打电话，先拨号（请求地址）→说你的事（输入参数）→听对方回复（返回结果）。该图与上下文紧密相关，直观呈现了API通信的三要素概念。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DA4hbLZd5ozMOWxInvnc9cCfn3c) · `DA4hbLZd5ozMOWxInvnc9cCfn3c`
...
> 图片展示了请求地址的拆解，以“https://api.deepseek.com/chat/completions”为例。其中“https://”是通信协议，即程序之间说话的“语言”；“api.deepseek.com”是域名，找到DeepSeek的服务器；“/chat/completions”是接口路径，定位到“对话”功能。底部总结为：定位到某台服务器的某个功能，就像找到某栋楼（域名）里的某个房间（接口路径）。该图与上下文紧密相关，直观说明了请求地址中各部分的作用及关系。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DTACbUO6iooRlzx4DFZcUgAXn2d) · `DTACbUO6iooRlzx4DFZcUgAXn2d`
...
> [!abstract]- 🖼 图片展示的是DeepSeek开放平台的“用量信息”页面。左侧菜单栏中“A
> 图片展示的是DeepSeek开放平台的“用量信息”页面。左侧菜单栏中“API keys”被红色框突出显示。页面上方显示“所有日期均按UTC时间显示，数据可能有5分钟延迟”。中间区域有“充值余额”和“本月消费”两项，充值余额为99.84元，本月消费为0元，均有红色框突出显示。页面底部有“接口文档”“常见问题”“产品定价”“实施集成”等选项。该图片与上文获取DeepSeek API Key的操作步骤相关，直观呈现了API Key所在位置及用量信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/L1cBb5r4woo08DxLEZIcwYTPnNh) · `L1cBb5r4woo08DxLEZIcwYTPnNh`

1. **在左侧菜单找到 API Keys**
2. 点击**创建 API Key**，系统会生成一串类似 `sk-xxxxxxxxxxxxxxxx` 的字符串

> [!abstract]- 🖼 图片展示的是DeepSeek开放平台的API keys页面。页面左侧有“
> 图片展示的是DeepSeek开放平台的API keys页面。页面左侧有“用量信息”“充值”“账单”等选项，当前选中“API keys”。右侧显示API keys列表，包含名称、Key、创建日期和最新使用日期等信息，其中“日常使用”Key为“sk - 3738f************56ce”，创建日期为2025 - 03 - 08，最新使用日期也为2025 - 03 - 08。页面底部有“创建API key”按钮。该图片与上下文关系紧密，直观呈现了获取DeepSeek API Key的操作界面及相关信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/EHZFbBT8IowxRwxOMLlc1tf6ngc) · `EHZFbBT8IowxRwxOMLlc1tf6ngc`
...
> 图片展示了DeepSeek API的相关信息。其中，base_url为https://api.deepseek.com，api_key需申请获取。特别指出，deepseek-chat对应DeepSeek-V3.2的非思考模式，deepseek-reasoner对应DeepSeek-V3.2的思考模式。此外，出于与OpenAI兼容考虑，base_url也可设为https://api.deepseek.com/v1。该图片与上下文紧密相关，是对上文提到的DeepSeek API使用与OpenAI兼容的API格式等内容的补充说明。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/DYTNb8zHCoUE1ixIyUccv9sdnWb) · `DYTNb8zHCoUE1ixIyUccv9sdnWb`
...
> [!abstract]- 🖼 图片展示了调用对话API的示例脚本，用于访问DeepSeek API。脚
> 图片展示了调用对话API的示例脚本，用于访问DeepSeek API。脚本使用curl命令，请求地址为https://api.deepseek.com/chat/completions，需添加Content-Type和Authorization头信息，其中Authorization值为Bearer后的API Key。请求体为JSON格式，包含model、messages（系统和用户信息）和stream（false）参数。该图片与上下文紧密相关，直观呈现了调通API时在文档里找到请求地址、参数格式、调用示例等操作后的具体实现方式。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/XM9nb0dZeocvq5xRfgucrPqenkh) · `XM9nb0dZeocvq5xRfgucrPqenkh`
...
> 图片展示的是用curl命令调用DeepSeek API的示例代码。代码中关键部分是将`${DEEPSEEK_API_KEY}`替换为API Key，该部分用红色框突出显示。上下文提到在执行命令前需将API Key替换至此处，此图直观呈现了替换后的完整命令格式，帮助用户理解如何在终端中正确输入命令以调用API。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/JYRxbSYnboYE81x2fUIcPewpnVc) · `JYRxbSYnboYE81x2fUIcPewpnVc`
...
> 图片展示了使用curl命令调用DeepSeek API后的输出结果。命令中包含请求地址、权限验证、输入参数等信息。输出结果中，红色箭头指向部分显示了API的响应内容，其中“content”字段显示了DeepSeek的回答，即“你好，我是我，我是DeepSeek，一个由深度求索公司创造的AI助手，致力于用热情和细心为你提供帮助！😊”。该图片图片与上文介绍的curl命令调用API及拆解命令内容相呼应，直观呈现了调通API后的结果。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/YQVmbtpuFoTNLFxYqypc1yUXnYD) · `YQVmbtpuFoTNLFxYqypc1yUXnYD`
...
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/BZRpbdOmcoP0Ifxn2Ywcoh7enCg) · `BZRpbdOmcoP0Ifxn2Ywcoh7enCg`
...
> [!warning]- 🎬 视频（`video/mp4`）
> 原文件未迁移 · [在飞书中打开](https://feishu.cn/file/HfUZbNOtwok0PLx3Z2JchBl3nch) · `HfUZbNOtwok0PLx3Z2JchBl3nch`
