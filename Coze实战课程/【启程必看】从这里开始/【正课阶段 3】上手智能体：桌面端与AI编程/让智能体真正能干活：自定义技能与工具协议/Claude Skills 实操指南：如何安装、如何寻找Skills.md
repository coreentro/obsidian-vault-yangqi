...
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/DyMPwfQHuiA2ghkoJ6FctM18n8f
node_token: DyMPwfQHuiA2ghkoJ6FctM18n8f
obj_token: JBmjd1T8toXPxHxlcGJcJTYpnpf
obj_type: docx
...
> [!abstract]- 🖼 图片展示的是在终端中输入`whoami`命令后的结果。终端显示用户名为`
> 图片展示的是在终端中输入`whoami`命令后的结果。终端显示用户名为`lmh`，且用户名被红色框线突出显示。该图片与文档中“不知道自己的用户名？打开终端输入`whoami`，显示的就是”这一内容相关，直观呈现了在终端输入`whoami`命令后显示用户名的场景，帮助用户了解如何在终端中查看自己的用户名，以便在后续手动安装Claude Skills时确定用户根目录下的技能文件夹路径。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/MOGUbuqZNoBRGFxQ9mBcQl47nuh) · `MOGUbuqZNoBRGFxQ9mBcQl47nuh`
...
> [!abstract]- 🖼 图片展示了GitHub上“anthropics/skills”仓库的页面
> 图片展示了GitHub上“anthropics/skills”仓库的页面。页面右上角有“Code”按钮，点击后弹出下拉菜单，其中“Download ZIP”按钮被红色箭头突出显示。页面左侧目录中，“skills”文件夹被红色框线圈出。该图片对应文档中“手动安装”步骤里下载Skill文件的第一种方式，即访问https://github.com/anthropics/skills，点击绿色“Code”按钮后选择“Download ZIP”下载整个git项目，再从中找到所需Skill。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/Mhn0bA88ioMNPHxaAPicKSd7nAb) · `Mhn0bA88ioMNPHxaAPicKSd7nAb`
