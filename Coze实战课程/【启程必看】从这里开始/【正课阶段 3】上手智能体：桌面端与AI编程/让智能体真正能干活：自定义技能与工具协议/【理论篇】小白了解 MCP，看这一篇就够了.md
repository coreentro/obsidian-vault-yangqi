...
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/OHtQwPfvuiiXrbkRSHicdwMXnje
node_token: OHtQwPfvuiiXrbkRSHicdwMXnje
obj_token: A7gudFuxqoxQ4Mxyk8McH3LCnAc
obj_type: docx
...
> [!abstract]- 🖼 图片展示了AI智能体与用户的一段对话示例。用户询问“你好呀，我吃了一桶泡
> 图片展示了AI智能体与用户的一段对话示例。用户询问“你好呀，我吃了一桶泡面”，AI回复并询问吃的是什么口味。接着用户回答“我吃的红烧牛肉面”，AI基于上下文信息，回复红烧牛肉面是经典口味，并给出搭配建议。AI还提到自己知道用户吃的是红烧牛肉面，因在回答问题时会把前面记录作为背景信息。该图直观呈现了AI智能体在对话中利用上下文信息进行回复的场景，与上下文介绍的AI智能体包含大模型、规划、记忆、工具等内容相呼应。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/XEAfbZqZroVji1xvJxxcFiWrnFh) · `XEAfbZqZroVji1xvJxxcFiWrnFh`
...
> 图片展示了RAG（知识库）的流程。从文件上传开始，经文件解析、切片、构建索引等步骤，将信息存储于数据库。用户提问后，通过query改写、路由、向量化等环节，最终由LLM输出答案。流程中还涉及排序、多路召回、LLM等关键步骤，如LLM使用prompt模板，排序使用rerank模型，多路召回包含语义检索和关键词检索。该图与文档中介绍RAG流程的内容相契合，直观呈现了其工作原理。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/SBlSb4PAEoiuntxwFXvc0uu5n6c) · `SBlSb4PAEoiuntxwFXvc0uu5n6c`
...
> [!abstract]- 🖼 图片展示了AI智能体的典型公式架构。其中，Agent（智能体）处于核心位
> 图片展示了AI智能体的典型公式架构。其中，Agent（智能体）处于核心位置，其左侧连接着Tools，包含Calendar()、Calculator()、CodeInterpreter()、Search()等多种工具；上方为Memory，分为Short - term memory和Long - term memory；右侧是Planning，涵盖Reflection、Self - critics等方面。下方为Action。该图直观呈现了AI Agent = LLM + Planning + Memory + Tools的构成关系，与上文介绍的AI智能体公式内容相呼应，形象地说明了大模型调用不同工具等情况。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/RAytbiSzZoMEXoxkRMEcSHBoneb) · `RAytbiSzZoMEXoxkRMEcSHBoneb`
...
> [!abstract]- 🖼 图片展示了豆包网站的界面，左侧有“新对话”“AI搜索”“帮我写作”等选项
> 图片展示了豆包网站的界面，左侧有“新对话”“AI搜索”“帮我写作”等选项，右侧显示“早上好，大圣”。红色箭头指向左侧“新对话”选项，旁边文字说明“当前这个页面就是客户端，也就是用户可以操控的页面或者软件”。下方红框内文字指出“这是一条即将发给服务端的消息，而我们当前的页面就是客户端”。图片与上下文紧密相关，通过豆包网站界面直观呈现了客户端的概念，帮助理解客户端是用户可操控的页面或软件，为后续讲解调用工具过程中的客户端-服务端模式打下基础。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/U4PxbrQZFo4gPBxl1lZcZobfnkc) · `U4PxbrQZFo4gPBxl1lZcZobfnkc`
...
> [!abstract]- 🖼 图片是Function Calling核心流程时序图，展示了Functi
> 图片是Function Calling核心流程时序图，展示了Function Calling的完整流程。用户提问后，客户端（应用）将问题及可用工具列表发送给服务 自动生成
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/Ha0wbZA1FoiP10x6SSBcRhF7n6l) · `Ha0wbZA1FoiP10x6SSBcRhF7n6l`
...
> 图片展示了MCP出现前后的集成情况。左侧“Without MCP”中，ChatGPT、Claude、Gemini等AI工具与Database、Email、CRM等工具间有大量复杂连接，呈现为“M×N”集成；右侧“With MCP”中，MCP作为中心节点，将AI工具与多种工具连接，简化为“M+N”集成。此图直观呈现MCP简化接口调用方式，降低维护与扩展成本，解决“N*M”集成问题，与上下文介绍的MCP出现前接口调用方式千差万别、维护成本高的情况相呼应。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/M0BCbnUXHoAMGdx8zb5ca9GTnyf) · `M0BCbnUXHoAMGdx8zb5ca9GTnyf`
...
> [!abstract]- 🖼 图片展示了MCP架构中主机（Hosts）与服务器（Server）的关系。
> 图片展示了MCP架构中主机（Hosts）与服务器（Server）的关系。本地计算机内有Host，其下有MCP Client A、B、C，通过MCP协议与本地工具MCP Server A、B交互，MCP Server A、B分别连接数据源A、B。此外，还有远端工具MCP Server C，也连接数据源B。该图直观呈现了MCP协议在Host与Server间的数据传输，与上下文介绍的MCP核心角色及协作模式相呼应。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/E28Rb8nmCokkOIxWkPLcpMU0nFd) · `E28Rb8nmCokkOIxWkPLcpMU0nFd`
...
> 图片展示了扣子空间界面，上方有“写作”“PPT”“插图”“网页”等功能选项，右侧有一个卡通形象。下方“给我布置一个任务”输入框旁，有“搜索工具/专家”选项，可选择“全部”“工具”“专家”。图片右侧有红色箭头，标注“扣子空间就是Host，它可以调度多个MCP工具”，并指向下方的“高德地图”“飞常准”“音乐生成”等MCP工具。该图直观呈现了扣子空间作为Host，可调度MCP工具完成任务的功能。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/YycJb8q0posBguxDhH9ca4wRnvk) · `YycJb8q0posBguxDhH9ca4wRnvk`
...
> [!abstract]- 🖼 图片展示了扣子空间的界面，上方有“写作”“PPT”“播客”“网页”等功能
> 图片展示了扣子空间的界面，上方有“写作”“PPT”“播客”“网页”等功能选项，右侧有一个粉色卡通形象。下方搜索栏提示“给我布置一个任务”，并有“试试@专家和工具”按钮。搜索结果中，“高德地图”被红色框和箭头突出显示，旁边文字说明“高德地图工具背后就是一个MCP Server在提供服务”。该图片与上下文关系紧密，通过具体示例直观呈现MCP Server在实际应用中的角色，帮助理解MCP协议在工具背后的运作。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/FoFUbylAPoBFeMxsy14cxnkmnic) · `FoFUbylAPoBFeMxsy14cxnkmnic`
...
> 图片展示了MCP架构，笔记本电脑主体内有MCP客户端（client.py），外部有多个MCP服务器，分别连接远程服务、本地数据源等。右侧是MCP主机，包含Claude等。左侧笔记本电脑接口处有USB插口，连接着多个外设。此图与上下文紧密相关，通过笔记本电脑与扩展坞类比，直观呈现MCP协议中客户端与服务器的对应关系，帮助理解客户端是连接主机和服务器的桥梁，与服务器1:1对应，帮助主机实现与外部数据交互。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/HuMYbeBLdorVMgxeMd1cb1zSnLh) · `HuMYbeBLdorVMgxeMd1cb1zSnLh`
