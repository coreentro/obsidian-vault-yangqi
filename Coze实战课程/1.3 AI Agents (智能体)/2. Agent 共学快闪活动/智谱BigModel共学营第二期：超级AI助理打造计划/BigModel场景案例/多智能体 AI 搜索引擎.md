---
title: "多智能体 AI 搜索引擎"
feishu_url: https://axsppz4oyvj.feishu.cn/wiki/N9Zyw6BhWiqze8k6wRocpc5vnEc
node_token: N9Zyw6BhWiqze8k6wRocpc5vnEc
obj_token: DcgHdbJ8zo7zYaxSJHwcNYosndg
obj_type: docx
...
> 图片展示的是BigModel智谱AI大模型开放平台的GLM-4-Flash相关内容。背景为蓝紫色渐变，右下角有蓝色六边形图案。图片上方有“BigModel”标识，中间大字“GLM-4-Flash”，下方文字为“免费 | 好用 | 高并发”，最下方是“让百万开发者实现Tokens自由”。该图片与文档中介绍的多智能体AI搜索引擎方案框架相关，可能是在说明GLM-4-Flash作为大模型在该方案中的应用或优势。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/IKCZbtHLco3zkXxrkP1cu1SBn7e) · `IKCZbtHLco3zkXxrkP1cu1SBn7e`

## 场景介绍

当前热门的AI搜索，能够深度搜索并总结内容，并生成思维导图，对于各类调研分析工作非常实用。如果我们也希望在自己的系统中集成类似的能力，可以通过多智能体协作来实现。

## 方案框架

实际场景中，比如用户需要深度调研开源技术方案，生成报告的同时制作成思维导图。AI搜索方案可以这样设计：

> [!abstract]- 🖼 图片展示了多智能体AI搜索引擎的搜索方案流程。用户提出任务后，首先使用工
> 图片展示了多智能体AI搜索引擎的搜索方案流程。用户提出任务后，首先使用工具API Web-Search-Pro进行快速搜索，获取初步信息。接着，通过模型API GLM-4 - 0620拆分问题，将用户问题分解为多个子搜索任务。各子任务分别由智能体（如Elasticsearch、OpenSearch等）负责搜索，再由智能体（如GPT - 4、Qwen等）进行深度搜索。最后，智能体负责生成搜索报告，智能体负责生成PPT报告，应用汇总报告并进行思维导图。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/MzuBbhh8YoYxg1xoo8Zczvqhnke) · `MzuBbhh8YoYxg1xoo8Zczvqhnke`

## 方案详情

### 第一步，快速搜索补充参考信息

首先需要根据用户的任务<调研市场上主流的开源搜索引擎技术架构>，使用搜索工具补充更多的信息。这里我们使用工具 API Web-Search-Pro，具体[参考文档](https://bigmodel.cn/dev/api/search-tool/web-search-pro)。
...
然后我们需要使用大模型来帮助我们规划，把用户问题拆分成若干子搜索任务，并转换为JSON格式。这里我们通过 GLM-4-0520 的模型来分析，参考 [API文档](https://bigmodel.cn/dev/api/normal-model/glm-4)。JSON格式处理，我们参考[JSON工具](https://bigmodel.cn/dev/howuse/jsonformat)。
...
AI搜索智能体不仅具备联网搜索的能力，还能够自主分析并进行多轮搜索任务。智能体API的调用方式，具体[参考文档](https://bigmodel.cn/dev/api/qingyanassistant/assistantapi)。

> [!abstract]- 🖼 图片展示的是AI搜索界面，上方有“AI搜索”字样及下拉箭头。中间是一个带
> 图片展示的是AI搜索界面，上方有“AI搜索”字样及下拉箭头。中间是一个带有放大镜和绿色勾形的蓝色圆形图标。下方文字介绍AI搜索为连接全网内容、精准搜索、快速分析并总结的智能助手，其开发者为智谱清言。该图片与文档中介绍智能体API调用方式，特别是智能体“AI搜索”相关内容紧密相关，直观呈现了智能体的标识及功能介绍。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/GQjPbcbsooCdmKx1yPWcVVqOnvf) · `GQjPbcbsooCdmKx1yPWcVVqOnvf`
...
智能体能调用各种插件，除了思维导图以外，还有流程图、PPT工具等，你可以根据工作需要选择。智能体API的调用方式，具体[参考文档](https://bigmodel.cn/dev/api/qingyanassistant/assistantapi)。

> [!abstract]- 🖼 图片展示的是“思维导图 MindMap”插件界面。画面中心是一个卡通人物
> 图片展示的是“思维导图 MindMap”插件界面。画面中心是一个卡通人物，周围环绕着多个图标，如云朵、地球、书籍等。下方文字为“思维导图 MindMap”，并配有“告别整理烦恼，任何复杂概念秒变脑图。”的说明。图片右上角有“思维导图 MindMap”字样及下拉箭头。该图片与文档中介绍智能体能调用各种插件，可选择思维导图等插件的内容相关，直观呈现了思维导图插件的样式和功能介绍。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/IsMubzWyqoL3IZxZYWlc5uS7nuc) · `IsMubzWyqoL3IZxZYWlc5uS7nuc`
...
> [!abstract]- 🖼 图片展示的是咨询大模型解决方案专家的宣传内容。左侧背景为蓝色渐变，有类似
> 图片展示的是咨询大模型解决方案专家的宣传内容。左侧背景为蓝色渐变，有类似水滴的透明球体图案。右侧上方有“咨询大模型解决方案专家”文字，下方是“扫描二维码，助力AI应用落地”字样。右侧还配有二维码，二维码旁标注“BigModel”。该图片位于文档结尾处，是对文档内容的补充说明，意在引导读者扫描二维码，获取更多关于大模型解决方案的信息。
> 
> 原图未迁移 · [飞书原图](https://feishu.cn/file/Y2smbCvctoHBf6xQ88Scs2YUn6g) · `Y2smbCvctoHBf6xQ88Scs2YUn6g`
